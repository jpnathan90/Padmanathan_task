...
class SWAPI
{
	...
	/**
     * @param bool $fresh
     * @return Endpoints\PeoplesEndpoint
     */
    public function peoples($fresh = false)
    {
        if (!isset($this->peoples) || $fresh) {
            $this->peoples = new Endpoints\PeoplesEndpoint($this->http, $this->logger, $this->mapper);
        }
        return $this->peoples;
    }
    ...
    public function getFromUri($uri)
    {
        if (preg_match("/\/api\/(\w+)\/(\d+)(\/|$)/", $uri, $matches) !== false) {
            switch (strtolower($matches[1])) {
				...
				case "people":
                    return $this->peoples()->get($matches[2]);
                case "films":
