...
function simpleValidation(form,doAlert){
    ...
    window.jQuery(form).submit(function () {
        ...
        if (o.err.length) {
			...
			var queryString = $('#frmExample').serialize();

            var saveData = $.ajax({
				  type: 'POST',
				  url: "padmanathan_addCharacter.php",
				  data: queryString,
				  dataType: "text",
				  success: function(resultData) {
					$('.frmBtn').removeClass('hide');
					$('.frmPrc').addClass('hide');
					$(".padmanathan_addCharacter").trigger("reset");
					alert(resultData);
					return true;
				  }
			});
...
function validateForm(form){
    ...
    window.jQuery(form).find('.required input,.required select,.required textarea,input.required, select.required, textarea.required').each(function () {
        ...
        if (((type === 'number' || me.hasClass('integer')) && !Validate.integer(me.val())) ||
...
window.jQuery(function(){
    ...
    window.jQuery('form.padmanathan_addCharacter').each(function(){
        simpleValidation(this,false);
    });
    // !Simple form validation with alert
    window.jQuery('form.padmanathan_addCharacter-alert').each(function(){
